...
class BookClass:
    def __init__(self):
        ...
        self.login_url = class_config_file['LOGIN_URL'].replace("'", "")
        self.book_class_url = class_config_file['BOOK_CLASS_URL'].replace("'", "")
        self.time_table_url = class_config_file['TIMETABLE_URL'].replace("'", "")
        self.cal_url = class_config_file['CAL_URL'].replace("'", "")
        self.time_table_file = self.utils.get_config()['FILES']['TIME_TABLE_DATA'].replace("'", "")
        self.class_dict = {}
        ...
        try:
            with session() as c:
                ...
                if len(classes_per_day) == 0:
                    log.error('Could not fetch the table data, function will wait 5 seconds and try again ( max number 6 times )')
                    raise Exception
        ...
        try:
            ...
            with session() as c:
                ...
                if len(c.cookies.items()) > 0:
                    ...
                    log.exception('Unable to open session; please check if the username and password is correct for user {0}'.format(user))
                ...
                if response.status_code == 200:
                    ...
                    if 'Something went wrong' in response.text:
                        log.error('Unable to book class ({0} on {1} at {2} for user: {3}) - something went wrong'.format(
                            class_name,
                            class_day,
                            class_time,
                            user
